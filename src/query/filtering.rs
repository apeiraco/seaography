    ...
    T: EntityTrait,
{
    if let Some(filters) = filters {
        ...
        recursive_prepare_condition::<T>(context, resolver_context, &filters)
    } else {
        Ok(Condition::all())
    }
}
    ...
    T: EntityTrait,
{
    ...
    let condition = T::Column::iter().try_fold(
        ...
        |condition, column: T::Column| -> SeaResult<Condition> {
            ...
            if let Some(filter) = filter {
                ...
                filter_types_map_helper.prepare_column_condition::<T>(resolver_context, condition, &filter, &column)
            } else {
                Ok(condition)
            }
        },
    ...
    let condition = if let Some(and) = filters.get("and") {
        ...
        let nested_condition = filters.iter().try_fold(
            ...
            |condition, filters: ValueAccessor| -> SeaResult<Condition> {
                let filters = filters.object()?;
                Ok(condition.add(recursive_prepare_condition::<T>(context, resolver_context, &filters)?))
            },
        )?;
    ...
    let condition = if let Some(not) = filters.get("not") {
        let nested_condition = recursive_prepare_condition::<T>(context, resolver_context, &not.object()?)?;
